...
func _update_camera(delta: float) -> void:
	...
	if _shake_amount > 0.0:
		desired_position += Vector3(
			sin(Time.get_ticks_msec() * 0.047) * _shake_amount,
			cos(Time.get_ticks_msec() * 0.061) * _shake_amount * 0.45,
			sin(Time.get_ticks_msec() * 0.073) * _shake_amount
		)
