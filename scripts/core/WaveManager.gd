...
func _start_boss_wave(spawner: EnemySpawner) -> void:
	...
	active_support_spawn_count = _apply_spawn_multiplier(active_support_spawn_count)
...
func _apply_spawn_multiplier(spawn_count: int) -> int:
	if spawn_count <= 0:
		return 0
	if _active_level_data == null or _active_level_data.difficulty == null:
		return spawn_count
	return maxi(roundi(float(spawn_count) * maxf(_active_level_data.difficulty.spawn_multiplier, 0.1)), 1)

func _apply_pressure_rating(speed_bonus: float) -> float:
	if _active_level_data == null or _active_level_data.difficulty == null:
		return speed_bonus
	return speed_bonus * maxf(_active_level_data.difficulty.pressure_rating, 0.1)
