...
func _ready() -> void:
    ...
    game_manager.game_over_changed.connect(_on_gameplay_end_state_changed)
    game_manager.victory_changed.connect(_on_gameplay_end_state_changed)
...
func _on_gameplay_end_state_changed(is_active_end_state: bool) -> void:
    if not is_active_end_state:
        return
    _clear_guards()

func _clear_guards() -> void:
    for guard_id in _active_guards.keys():
        var guard := _active_guards[guard_id] as Node3D
